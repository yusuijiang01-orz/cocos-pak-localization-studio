...
function setWorkflowDisabled(disabled){for(const id of ['exportXlsx','importXlsxPolish','exportFullXlsx','importFullXlsx','apiReview','reReviewSelected','exportTsvCsv','mergeTsvCsv','splitMergedTsvCsv','translateTsvCsv','modelTranslateTsvCsv','apiTranslateMergedCsv','importTsvCsv','exportUntranslated','ollamaTranslate','importUntranslated','safePcMerge','autoModelBuild','buildPak','pushGithubPak']){const el=$('#'+id);if(!el)continue;if(id==='autoModelBuild'&&workflowRunning){el.disabled=false;continue}if(id==='apiReview'&&reviewRunning){el.disabled=false;continue}el.disabled=disabled}if(!disabled)updateReviewSelectionControls()}
...
$('#ollamaTranslate').onclick=async()=>{const pak=selectedPakOrWarn();if(!pak||!project)return;const config=await apiConfigForRun();const profiles=config?.profiles||[];const profile=profiles.find(p=>p.id==='ollama'||String(p.name||'').trim().toLowerCase()==='ollama')||profiles.find(p=>/:(11434|11435)(?:\/|$)/.test(String(p.baseUrl||'')))||profiles[0]||{};const model='translategemma:4b';const modelBucketCap=/qwen/i.test(model)?25:50;const configured=Number(profile.batchSize);const bucketSize=Number.isFinite(configured)&&configured>0?Math.min(modelBucketCap,Math.max(1,Math.trunc(configured))):modelBucketCap;if(!await appConfirm(`请选择“导出逐文件 XLSX”生成的 xlsx 文件夹。Ollama 会先读取 Studio 当前语言状态，把仍含越南文/中越混合的源文件排到最前；纯中文文件直接复用或跳过。随后读取同级 config：已完成且未修改的文件跳过，部分完成文件从 checkpoint 继续。\n\n模型：${model}\n每桶最多：${bucketSize} 行（长文本仍按约 3000 字自动拆桶）`,{title:'开始越南文优先队列',confirmText:'选择 XLSX 文件夹'}))return;ollamaActiveThink=profile.think===true;const btn=$('#ollamaTranslate');btn.disabled=true;setWorkflowDisabled(true);startTranslationClock();setTranslationProgress(0,'正在识别仍含越南文的源文件并建立优先队列');try{const res=await window.studio.ollamaTranslate({project,pak,bucketSize,think:profile.think===true});if(res?.canceled){hideTranslationProgress();stopTranslationClock();return}if(!res?.ok){hideTranslationProgress();stopTranslationClock();refreshRecordsFromResult(res);alert(res?.error||'Ollama 文件夹翻译失败',{title:'Ollama 链路未完成',tone:'error'});return}refreshRecordsFromResult(res);const r=res.report||{},imp=res.importReport||{},sync=res.resourceSync?.report||{},rejected=Number(imp.rejected||0)+Number(res.autoRejected||0);setTranslationProgress(100,r.stopped?'文件夹队列已停止':(rejected?'翻译完成，坏格式译文已保留原文':'文件夹队列翻译完成'));stopTranslationClock();$('#progress').textContent=`Ollama 队列：越南文优先 ${r.prioritized_vietnamese_files||0} 个，完成 ${r.completed_files||0}/${r.total_files||0}，跳过 ${r.skipped_completed_files||0}，剩余 ${r.remaining_files||0}`;alert(`Ollama 文件夹队列${r.stopped?'已停止':'已完成'}。\n\nXLSX：${res.folder||r.folder||''}\n配置与检查点：${res.configFolder||r.config_dir||''}\n文件总数：${r.total_files||0}\n仍含越南文并优先处理：${r.prioritized_vietnamese_files||0}\n累计完成：${r.completed_files||0}\n本次直接跳过已完成：${r.skipped_completed_files||0}\n本次实际进入处理：${r.processed_files||0}\n剩余文件：${r.remaining_files||0}\n失败文件：${r.failed_files||0}\n安全导回变化：${imp.changed||0} 条\n安全拒收并保留原文：${rejected} 条\n参与资源回写：${sync.modified_records||0} 条\n生成资源文件：${sync.changed_file_count||0} 个\n最终回写跳过：${sync.skipped_count||0} 条`,{title:rejected?'Ollama 完成，部分译文被安全拒收':'Ollama 翻译与资源同步完成',tone:rejected?'warning':'success'})}finally{setWorkflowDisabled(false);btn.disabled=false}};
